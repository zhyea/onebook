...
//是否开启首页缓存
$indexOn = get_opt('ob_Cache_index_on');
define('INDEXON', $indexOn);

//是否开启分类页缓存
$cateOn = get_opt('ob_Cache_cate_on');
define('CATEON', $cateOn);

//是否开启页面缓存
$PageOn = get_opt('ob_Cache_page_on');
...
if (!function_exists('htmlCacheDelNb')) {
    ...
    function htmlCacheDelNb($post_ID)
    {
        ...
        ob_logs(json_encode($postRes));
        foreach($postRes as $post){
            if($post->ID != ''){
                $url = get_permalink($post->ID);
                ob_logs('删除相关文章，相关文章URL：'.$url);
                DelCacheByUrl($url);
            }
        }
        return true;
